"""
Smart Transit & Commuter Agent TaskList.

Persisted at ~/.smart_transit_agent/tasks.json. Both the agent (via tool functions
...
def tasks_done(task_id: int = 0, item_id: int = 0, id: int = 0) -> str:
    """Mark a task as completed by its ID.

    Args:
        task_id: The ID of the task shown in the UI.

    Returns:
        Confirmation or error message.
    """
    actual_id = task_id or item_id or id
    if not actual_id:
        return "Error: Please provide a valid task_id."
        
    t = TaskList.load()
    item = t._get(actual_id)
    if not item:
        return f"Error: task #{actual_id} does not exist."
        
    msg = ""
    if item.status == "done":
        msg = f"Task #{task_id} is already marked as done."
    elif t.complete(actual_id):
        t.save()
        msg = f"Marked #{actual_id} as done: {item.text}"
    else:
        return f"Error: could not complete task #{actual_id}."
