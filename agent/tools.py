...
Task tools (no approval, write to ~/.smart_transit_agent/tasks.json):
...
def create_artifact(name: str, content: str) -> str:
    ...
    artifact_dir = Path.home() / ".smart_transit_agent" / "artifacts"
...
def get_live_traffic(location: str = "", origin: str = "", destination: str = "") -> str:
    """
    Check the current live traffic congestion for a specific location or route.
    """
    # Mocking for the hackathon
    loc = location or f"{origin} থেকে {destination}"
    location_lower = loc.lower()
    if "gulshan" in location_lower or "banani" in location_lower or "গুলশান" in location_lower or "বনানী" in location_lower:
        return f"{loc}-এ তীব্র ট্রাফিক জ্যাম রয়েছে। গড় গতি ৫ কিমি/ঘন্টা।"
    elif "dhanmondi" in location_lower or "ধানমন্ডি" in location_lower:
        return f"{loc}-এ মাঝারি ট্রাফিক রয়েছে। কিছুটা দেরি হতে পারে।"
    else:
        return f"{loc}-এর ট্রাফিক বর্তমানে স্বাভাবিক।"

def get_ride_estimate(service: str, origin: str, destination: str) -> str:
    """
    Get a price and time estimate for a ride-sharing service (e.g., Pathao, Uber).
    """
    # Mocking for the hackathon
    return f"{origin} থেকে {destination} পর্যন্ত {service}-এর সম্ভাব্য খরচ ও সময়:\nভাড়া: ৳২৫০ - ৳৩৫০\nসময়: পিকআপের জন্য ৫ মিনিট, যাত্রার সময় ৪০ মিনিট।"
...
TOOL_REGISTRY: dict[str, object] = {
    ...
    "get_live_traffic":  get_live_traffic,
    "get_ride_estimate": get_ride_estimate,
}
